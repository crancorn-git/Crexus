const navItems = [
  ...
  { key: 'leaderboard', label: 'Ladder', hint: 'Ranked players' },
];

export function BackButton({ onClick, label = 'Back to Scout Search' }) {
  if (!onClick) return null;
  return (
    <button onClick={onClick} type="button" className="crexus-back-btn">
...
export default function CrexusShell({ activeView, onNavigate, children }) {
  return (
    <div className="min-h-screen text-gray-200 lg:grid lg:grid-cols-[248px_1fr]">
      <aside className="border-b border-white/10 bg-[#090a0e]/95 px-4 py-4 lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r lg:px-5 lg:py-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="text-lg font-black uppercase tracking-[0.18em] text-white">Crexus</div>
            <div className="text-[10px] font-black uppercase tracking-[0.22em] text-red-300">v1.1.0</div>
          </div>
        </div>

        <nav className="mt-5 flex gap-2 overflow-x-auto pb-1 lg:mt-8 lg:flex-col lg:overflow-visible lg:pb-0" aria-label="Crexus tools">
          {navItems.map((item, index) => {
            ...
            return (
              <button
                ...
                className={`min-w-max rounded-2xl border px-4 py-3 text-left transition lg:min-w-0 ${index === 0 ? 'mb-0 lg:mb-3' : ''} ${
                  active
